...
function getProperties(map) {
  return {
    ...
    zoom: (zoom => (zoom ? parseInt(zoom, 10) : 10))(map.getAttribute('data-zoom')),
  ...
  const infoTemplate = `
    <div class="info">
        ...
        <ul>
            ${
              props.location != null
                ? `<li><i class="fa fa-map-marker"></i>${props.location}</li>`
                : ''
            }
            ${
              props.website != null
                ? `<li><i class="fa fa-globe"></i><a href="${props.website}" target="_blank">${props.website}</a></li>`
                : ''
            }
