...
function getPosts() {
  return new Promise(resolve => {
    ...
    if (posts == null) {
      const url = (() => {
        const contentApiAccessKey = document.querySelector(
          'meta[property="content-api:access-key"],meta[name="content-api:access-key"]'
        );
    ...
    .then(x => {
      ...
      if (matched.length > 0) {
        matched.forEach(match => {
          const post = posts.find(p => match.ref === p.id);

          const template = `
                <article class="post preload-background" data-image="${
            post.feature_image
            }" data-resize="true">
